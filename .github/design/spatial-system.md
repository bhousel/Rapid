...
5. **Support richer, pre-computed geometry.** `GeometryPart` should compute derived geometry (e.g. buffers around shapes) at `setData` time so the data is ready to index the moment `spatial.replaceData`/`replaceData` is called.
...
- `editor_base` — the unedited downloaded data (the base graph).
- `editor_staging` — the work-in-progress edit. Useful for live feedback while the user is drawing.
- `editor_stable` — the latest committed edit. The data validators should base their findings on.
...
- before `merge` → update `editor_base` (new downloaded data streams in)
- before `stablechange` → update `editor_stable` (only changes when the user commits an edit)
- before `stagingchange` → update `editor_staging` (only changes as the user performs work-in-progress edits)

All three caches are maintained. Note the differing update cadences: `editor_base` updates continuously as data streams in, while `editor_stable` and `editor_staging` only update when the user actually edits.
...
interface SpatialCache {
  ...
  boxes: Map<BoxID, SpatialItem>;
  rbush: RBush<SpatialItem>;
}
```

`SpatialItem.contents` becomes generic (`unknown` / a type param) plus a `kind` discriminator so callers can tell what they got back. The existing `addData`/`addTiles`/`getVisibleItems`/etc. become thin wrappers over a generic index API (`index('data')`, `index('tiles')`), so this is a **non-breaking refactor first** — public behavior is unchanged, the internals just stop being hardcoded.
...
- `spatialIDForGraph` maps a graph to `editor_base` / `editor_stable` / `editor_staging` by identity (defaulting to staging). `intersects`/`waySegments` resolve hits through the *passed* graph, so a slight cache/graph mismatch degrades gracefully (missing entities are filtered out) rather than throwing.
...
- **`EditSystem`** — owns OSM topology/history. Translates graph `Difference`s into spatial updates, generates segments, owns the `editor_base`/`editor_stable`/`editor_staging` caches, and exposes the OSM spatial queries.
