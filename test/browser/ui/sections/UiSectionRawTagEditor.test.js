describe('UiSectionRawTagEditor', () => {
  ...
  function render(tags) {
    ...
    $container.call(rawTagEditor.render);
  }


  it('creates input elements for each key-value pair', () => {
    render({ highway: 'residential' });
    assert.isNotEmpty($container.selectAll('input[value=highway]'));
    assert.isNotEmpty($container.selectAll('input[value=residential]'));
  });

  it('creates a pair of empty input elements if the entity has no tags', () => {
    render({});
    assert.isEmpty($container.select('.tag-list').selectAll('input.value').property('value'));
    assert.isEmpty($container.select('.tag-list').selectAll('input.key').property('value'));
  });

  it('adds tags when clicking the add button', () => {
    render({ highway: 'residential' });
    const target = $container.selectAll('button.add-tag').node();
    target.dispatchEvent(new MouseEvent('click'));
    return delay(40)
      .then(() => {
        assert.isEmpty($container.select('.tag-list').selectAll('input').nodes()[2].value);
        assert.isEmpty($container.select('.tag-list').selectAll('input').nodes()[3].value);
      });
  });
  ...
  it('removes tags when clicking the remove button', done => {
    ...
    const button = $container.selectAll('button.remove').node();
    button.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true }));
  });

  it('adds tags when pressing the Tab key on last input.value', () => {
    render({ highway: 'residential' });
    assert.lengthOf($container.selectAll('.tag-list li').nodes(), 1);
    const input = $container.select('.tag-list li:last-child input.value').node();
    input.dispatchEvent(new KeyboardEvent('keydown', { key: 'Tab', keyCode: 9 }));
    return delay(40)
      .then(() => {
        assert.lengthOf($container.selectAll('.tag-list li').nodes(), 2);
        assert.isEmpty($container.select('.tag-list').selectAll('input').nodes()[2].value);
        assert.isEmpty($container.select('.tag-list').selectAll('input').nodes()[3].value);
      });
  });
