...
describe('actionMergePolygon', () => {
  ...
  it('creates a multipolygon from two closed ways', () => {
    ...
    assert.instanceOf(r, Rapid.OsmRelation);
    assert.strictEqual(r.geometry(result), 'area');
    assert.isTrue(r.isMultipolygon());
    assert.lengthOf(r.members, 2);

    const m0 = r.memberById('w0');
    assert.strictEqual(m0.role, 'outer');
    assert.strictEqual(m0.type, 'way');

    const m1 = r.memberById('w1');
    assert.strictEqual(m1.role, 'inner');
    assert.strictEqual(m1.type, 'way');
  });
  ...
  it('creates a multipolygon from a closed way and a multipolygon relation', () => {
    ...
    assert.instanceOf(graph2, Rapid.Graph);
    const result = Rapid.actionMergePolygon(['r', 'w2'])(graph2);
    assert.instanceOf(result, Rapid.Graph);

    const r = result.hasEntity('r');
    assert.instanceOf(r, Rapid.OsmRelation);
    assert.lengthOf(r.members, 3);
  });
  ...
  it('creates a multipolygon from two multipolygon relations', () => {
    ...
    assert.instanceOf(graph3, Rapid.Graph);
    const result = Rapid.actionMergePolygon(['r', 'r2'])(graph3);
    assert.instanceOf(result, Rapid.Graph);

    // Delete the other relation
    assert.isNotOk(result.hasEntity('r2'));

    const r = result.hasEntity('r');
    assert.instanceOf(r, Rapid.OsmRelation);

    assert.strictEqual(r.memberById('w0').role, 'outer');
    assert.strictEqual(r.memberById('w1').role, 'inner');
    assert.strictEqual(r.memberById('w2').role, 'outer');
    assert.strictEqual(r.memberById('w5').role, 'outer');
  });
  ...
  it('merges no tags from unclosed outer ways', () => {
    ...
    assert.instanceOf(result, Rapid.Graph);
    assert.strictEqual(result.entity('w3').tags.natural, 'water');
    assert.strictEqual(result.entity('r1').tags.natural, undefined);
  });
  ...
  it('merges no tags from inner ways', () => {
    ...
    assert.instanceOf(result, Rapid.Graph);
    assert.strictEqual(result.entity('w1').tags.natural, 'water');
    assert.strictEqual(result.entity('r').tags.natural, undefined);
  });
  ...
  it('doesn\'t copy area tags from ways', () => {
    ...
    const result = Rapid.actionMergePolygon(['w0', 'w1'], 'r')(graph2);
    assert.instanceOf(result, Rapid.Graph);
    assert.strictEqual(result.entity('r').tags.area, undefined);
  });
  ...
  it('extends a multipolygon with multi-way rings', () => {
    ...
    assert.strictEqual(r.memberById('w0').role, 'outer');
    assert.strictEqual(r.memberById('w2').role, 'outer');
    assert.strictEqual(r.memberById('w3').role, 'inner');
    assert.strictEqual(r.memberById('w4').role, 'inner');
  });
