...
export class UiMinimap {
  ...
  public viewMini: Viewport;
  public stage: any;          // PIXI.Container() that lives at the root of this scene
  public layer: any;          // Background Tile layer for the minimap

  protected _isHidden: boolean;
  protected _skipEvents: boolean;
  protected _gesture: string | null;
  protected _zDiff: number;
  protected _tStart: any;
  protected _keys: any;
  ...
  public render($parent: D3Selection | null = this.$parent): void {
    ...
    // add .map-in-map
    let $wrap: D3Selection = $parent.selectAll('.map-in-map')
      .data([0]);
    ...
    // add .supersurface
    let $supersurface: D3Selection = $wrap.selectAll('.supersurface')
      .data([0]);
    ...
    // add .surface
    let $surface: D3Selection = $supersurface.selectAll('.surface')
      .data([0]);
  ...
  protected _updateBoundingBox(): void {
    ...
    const mainPoints: Vec2[] = [[0, 0], [0, h], [w, h], [w, 0], [0, 0]];
  ...
  public initAsync(): Promise<void> {
    ...
    // Layer
    this.layer = new (PixiLayerBackgroundTiles as any)(miniScene, 'minimap-background');
   ...
   * @param  status - one of 'contextlost' or 'contextrestored'
   */
  protected _onGfxStatusChange(status: string): void {
   ...
   * @return Promise resolved when this component has completed reset and init
   */
  protected _resetAsync(): Promise<void> {
    const context = this.context;
    const gfx = context.systems.gfx!;
