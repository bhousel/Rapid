...
/**
 * `UiBearingControl` renders the compass button that shows the map's current
 * bearing (rotation) and resets it to north-up when clicked.
 */
export class UiBearingControl {
  ...
  public updateBearing(): void {
    ...
    ($button.selectAll(':scope > .icon use') as any)
