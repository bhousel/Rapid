...
export class UiPhotoViewer extends EventEmitter {
  ...
  public render($parent = this.$parent): void {
    ...
    // add .photoviewer
    let $viewer: D3Selection = $parent.selectAll('.photoviewer')
      .data([0]);
    ...
    this.$viewer = $viewer = $viewer.merge($$viewer) as D3Selection;
    ...
    for (const y of ['top', 'middle', 'bottom']) {
      for (const x of ['left', 'middle', 'right']) {
        ...
        $viewer
          ...
          .append('div')
          .attr('class', k)
          .on('touchstart touchdown touchend', (e: Event) => e.preventDefault());
   ...
   * @param  k - cell key, for example 'top-left', 'top-right', etc.
   * @return the resizer pointerdown function to attach to the cell
   */
  protected _buildResizer(k: string): (e: PointerEvent) => void {
    let pointerId: number | string;
    let startX: number, startY: number, rectW: number, rectH: number;
    ...
    const _pointerdown = (e: PointerEvent) => {
      ...
      const rect = (this.$viewer!.node() as HTMLElement).getBoundingClientRect();
      rectW = rect.width;
      rectH = rect.height;
     ...
     */
    const _pointermove = (e: PointerEvent) => {
      ...
      const mapRect = ($content.node() as HTMLElement).getBoundingClientRect();
     ...
     */
    const _pointerup = (e: PointerEvent) => {
      if (pointerId !== (e.pointerId || 'mouse')) return;
   ...
   */
  protected _onMapResize(): void {
    const $content = this.context.container().selectAll('.main-content');
    const $viewer = this.$viewer;
    if (!$content.size() || !$viewer) return;  // called too early?

    const mapRect = ($content.node() as HTMLElement).getBoundingClientRect();
    const viewerRect = ($viewer.node() as HTMLElement).getBoundingClientRect();
    const [maxW, maxH] = [mapRect.width - trimW, mapRect.height - trimH];
