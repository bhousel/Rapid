...
/**
 * `MoveOperation` puts the user into the move mode to reposition the selected features.
 */
export class MoveOperation extends AbstractOperation {
   ...
   * @param  selectedIDs - The entityIDs to move
   */
  public constructor(context: Context, selectedIDs: EntityID[]) {
    ...
    this.id = 'move';
    this.keys = [ l10n.t('shortcuts.command.move.key') ];
    this.title = l10n.t('operations.move.title');
  ...
  public run(): void {
    this.context.enter('move', { selection: { osm: this.selectedIDs }} );
  }
  ...
  public tooltip(): string {
    ...
    return disabledReason ?
      l10n.t(`operations.move.${disabledReason}`, { n: this.selectedIDs.length }) :
      l10n.t(`operations.move.description`, { n: this.selectedIDs.length });
  }
  ...
  public annotation(): string {
    ...
    if (this.selectedIDs.length === 1) {
      ...
      return l10n.t('operations.move.annotation.feature', { n: this.selectedIDs.length });
    }
  }
}
